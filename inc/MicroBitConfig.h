...
#else
#ifdef SOFTDEVICE_PRESENT
#define MICROBIT_BOOTLOADER_ADDRESS                                            \
  ((*(uint32_t *)MBR_BOOTLOADER_ADDR) != 0xFFFFFFFF                            \
       ? *(uint32_t *)MBR_BOOTLOADER_ADDR                                      \
       : NRF_UICR->NRFFW[0])
#else
...
#define MICROBIT_BOOTLOADER_SIZE                                               \
  (MICROBIT_CODESIZE * MICROBIT_CODEPAGESIZE - MICROBIT_BOOTLOADER_ADDRESS)
#endif
...
#else
#ifdef SOFTDEVICE_PRESENT
#define MICROBIT_MBR_PARAMS                                                    \
  ((*(uint32_t *)MBR_PARAM_PAGE_ADDR) != 0xFFFFFFFF                            \
       ? *(uint32_t *)MBR_PARAM_PAGE_ADDR                                      \
       : NRF_UICR->NRFFW[1])
#else
...
#define MICROBIT_BOOTLOADER_SETTINGS                                           \
  (MICROBIT_MBR_PARAMS + MICROBIT_CODEPAGESIZE)
#endif

// Defines where in memory persistent data is stored.
#ifndef MICROBIT_STORAGE_PAGE
#ifdef SOFTDEVICE_PRESENT
#define MICROBIT_STORAGE_PAGE                                                  \
  (MICROBIT_BOOTLOADER_ADDRESS - MICROBIT_CODEPAGESIZE * 3)
#else
...
#define MICROBIT_FDS_PAGE                                                      \
  (MICROBIT_BOOTLOADER_ADDRESS - MICROBIT_CODEPAGESIZE * 2)
#endif

#ifndef MICROBIT_DEFAULT_SCRATCH_PAGE
#define MICROBIT_DEFAULT_SCRATCH_PAGE                                          \
  (MICROBIT_BOOTLOADER_ADDRESS - MICROBIT_CODEPAGESIZE * 4)
#endif
...
#define MICROBIT_SOFTDEVICE_EXISTS                                             \
  ((*(uint32_t *)0x3004) == ((uint32_t)0x51B1E5DB))
#endif
...
#else
#define MICROBIT_APP_REGION_START                                              \
  (MICROBIT_SOFTDEVICE_EXISTS ? (*(uint32_t *)0x3008) : (uint32_t)0)
#endif
#endif
...
//
// FileSystem writeback cache size, in bytes. Defines how many bytes will be
// stored in RAM before being written back to FLASH. Set to zero to disable this
// feature. Should be <= MBFS_BLOCK_SIZE.
//
...
// This is recorded by the C/C++ linker, but the symbol name varies depending on
...
#define FLASH_PROGRAM_END                                                      \
  ((uint32_t)(&__etext) + (uint32_t)(&__data_end__) -                          \
...
// Set '1' to enable.
// n.b. This is known to be a feature that suffers compatibility issues with
// many BLE central devices.
...
// Open BLE links are not secure, but commonly used during the development of
// BLE services Set '1' to disable all secuity
...
// services MICROBIT_BLE_SECURITY_MODE is used instead of
// MICROBIT_BLE_SECURITY_LEVEL If MICROBIT_BLE_SECURITY_LEVEL and
// MICROBIT_BLE_SECURITY_MODE is defined, they must match as follows SEC_OPEN /
// OPEN_LINK:    1 SEC_JUST_WORKS   / NO_MITM:      2 SEC_MITM         /
// WITH_MITM:    3
//
...
#define __MICROBIT_CONFIG_SECURITY_MODE(x)                                     \
  __MICROBIT_CONFIG_CAT(__MICROBIT_CONFIG_, x)
#define __MICROBIT_CONFIG_SECURITY_MODE_IS(x)                                  \
  (__MICROBIT_CONFIG_SECURITY_MODE(MICROBIT_BLE_SECURITY_LEVEL) ==             \
   __MICROBIT_CONFIG_SECURITY_MODE(x))
...
// Afer this period, advertising will cease. Set to '0' for no timeout (always
// advertise).
...
// Valid values are in the range 0..7 inclusive, with 0 being the lowest power
// and 7 the highest power. Based on trials undertaken by the BBC, the radio is
// normally set to a low power level to best protect children's privacy.
...
// TODO: Update the range here once issue codal-microbit-v2#383 has been
// resolved https://github.com/lancaster-university/codal-microbit-v2/issues/383
#ifndef MICROBIT_RADIO_MAX_PACKET_SIZE
#define MICROBIT_RADIO_MAX_PACKET_SIZE 1025
#endif

// Versioning options.
// We use semantic versioning (http://semver.org/) to identify different
// versions of the micro:bit runtime. If this isn't available, it can be defined
// manually as a configuration option.
//
...
// Defines default behaviour of any stored user data when the micro:bit is
// reflashed. 0: No action is taken 1: Data is invalidated (any may be hard
// erased, as per policy of the respective file system used)
...
// 1: basic experience supported by dl.js in this repository hosted on
// microbit.org 2: "BBC micro:bit - the next gen" playground survey experience
